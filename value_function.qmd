...
\draw[thick,blue] (0,0.5) -- (3,-1.2) -- (6,0.55);

% Value points
\fill[blue] (0,0.5) circle (2pt) node[above right] {$V(0)=2.04$};
\fill[blue] (3,-1.2) circle (2pt) node[below right] {$V(1)=2.14$};
\fill[blue] (6,0.55) circle (2pt) node[above right] {$V(2)=2.25$};

% Cashflow arrows
\draw[->,red,very thick] (0,1.5) -- (0,0.7);
\node[above,red] at (0,1.5) {−\$100};

\draw[->,green,very thick] (3,-2) -- (3,-1.4);
\node[below,green] at (3,-2) {+\$50};

\draw[->,green,very thick] (6,-1) -- (6,0.3);
\node[below,green] at (6,-1) {+\$60};

% Zero line
\draw[dashed,gray] (-0.5,0) -- (9,0);
\end{tikzpicture}
\end{figure}
```
...
-   **Jumps** (discontinuities) occur at cashflow times
-   **Between cashflows**, the value changes smoothly due to interest accumulation
-   The **height** at any point shows your financial position at that moment
-   This project starts positive (good NPV), dips negative (after initial investment grows), then returns positive
:::
